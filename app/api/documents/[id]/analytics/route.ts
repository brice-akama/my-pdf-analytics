...
export async function GET(
  ...
  try {
    ...
    // ── Auth + plan via checkAccess ──────────────────────────────
    const access = await checkAccess(request)
    if (!access.ok) return access.response

    const analyticsLevel = getAnalyticsLevel(access.plan)
    // 'basic'    → free plan    → view counts only
    // 'full'     → starter/pro  → per-viewer breakdown, charts
    // 'advanced' → business     → everything + compliance
    ...
    // ── Core views (always computed — needed even for basic) ──────
    ...
    // ── BASIC plan response — return only counts, nothing personal ─
    // Free users see: total views, unique viewers, completion rate,
    // share count, last viewed date — and an upgrade prompt.
    // Everything else is stripped before it leaves the server.
    if (analyticsLevel === 'basic') {
      const completedSessionsCount = allSessions.filter((s: any) => {
        return (s.pagesViewed?.length || 0) >= document.numPages;
      }).length;

      const completionRate = totalViews
        ? Math.round((completedSessionsCount / totalViews) * 100)
        : 0;
      ...
      return NextResponse.json({
        success: true,
        analyticsLevel: 'basic',   // ← frontend reads this to show upgrade prompt
        analytics: {
          // ✅ Allowed on basic
          totalViews,
          uniqueViewers,
          ...
          averageTime: '—',
          avgTimePerSession: '—',
          avgTotalTimePerViewer: '—',

          // ✅ Empty/null — frontend shows locked state for these
          pageEngagement: [],
          topViewers: [],
          recipientPageTracking: [],
          bounceAnalytics: null,
          revisitData: null,
          intentScores: [],
          realTimeViewers: [],
          liveViewerCount: 0,
          devices: {},
          locations: [],
          eSignature: { totalRecipients: 0, completedCount: 0, completionRate: 0 },
          signatureFriction: [],
          declineReasons: [],
          declinePatterns: null,
          intentData: null,
    ...
    const uniqueLastPageViewers = new Set(
      lastPageLogs.map((l: any) => l.viewerId || l.email)
    ).size;
    ...
    const pageEngagement = await Promise.all(
      Array.from({ length: document.numPages }, async (_, i) => {
        ...
        const percentage = totalViews
          ...
          : 0;
        return { page: pageNum, views: percentage, avgTime, totalViews: pageViews, totalTime: totalTimeOnPage };
      })
    );

    const allViewerEmails: string[] = [];
    for (const share of shares) {
      allViewerEmails.push(...(share.tracking?.viewerEmails || []));
    }
    ...
    for (const vid of sessionViewerIds) {
      ...
      allViewerEmails.push(email ? email : `anon:${vid}`);
    }
      ...
      uniqueEmailsForTracking.map(async (emailOrId) => {
        ...
        const pageData = Array.from({ length: document.numPages }, (_, i) => {
          ...
          return {
            ...
            timeSpent: Math.min(timeOnPage, 1800),
    ...
    const intentScores = viewerIntents.map((v: any) => ({
      ...
      intentLevel: (v.intentScore || 0) >= 30 ? 'high' : (v.intentScore || 0) >= 15 ? 'medium' : 'low',
    ...
    const heatmapRaw = await db.collection('heatmap_events')
      .find({ documentId: id }).toArray();
    const heatmapByPage: Record<number, any> = {};
    for (let p = 1; p <= document.numPages; p++) {
      ...
      clicks.forEach((click: any) => {
        ...
        if (existing) { existing.count++; existing.x = (existing.x + click.x) / 2; existing.y = (existing.y + click.y) / 2; }
        else clickClusters.push({ x: click.x, y: click.y, count: 1 });
      });
      ...
      heatmapByPage[p] = {
        ...
        scrollStops,
        movementDensity: Object.values(densityGrid).sort((a, b) => b.density - a.density).slice(0, 100),
      };
    }

    const twoMinutesAgo = new Date(Date.now() - 2 * 60 * 1000);
    const liveViewers = await db.collection('viewer_presence')
      .find({ documentId: id, lastPing: { $gte: twoMinutesAgo } }).toArray();
    const realTimeViewers = liveViewers.map((v: any) => ({
      email: v.email, viewerId: v.viewerId, page: v.page, device: v.device, lastPing: v.lastPing,
    }));
    ...
    const bounceAnalytics = {
      ...
      bounceRate: totalTrackedRecipients > 0
        ? Math.round((bouncedCount / totalTrackedRecipients) * 100) : 0,
      avgEngagementTime,
    };

    const viewerEmailMap = new Map<string, { email: string; views: number; lastViewed: Date; totalTime: number; shares: string[] }>();
    for (const share of shares) {
      ...
      for (const email of viewerEmails) {
        ...
        const emailPageLogs = await db.collection('analytics_logs')
          .find({ documentId: id, email, action: 'page_view' }).sort({ timestamp: -1 }).toArray();
        const emailViewedLogs = await db.collection('analytics_logs')
          .find({ documentId: id, email, action: 'document_viewed' }).sort({ timestamp: -1 }).toArray();
        ...
        if (viewerEmailMap.has(email)) {
          ...
          if (!existing.shares.includes(share.shareToken)) existing.shares.push(share.shareToken);
        } else {
          viewerEmailMap.set(email, { email, views: viewCount, lastViewed: new Date(lastView), totalTime: timeSpent, shares: [share.shareToken] });
        }
      }
    ...
    const topViewers = Array.from(viewerEmailMap.values())
      ...
      .map(v => ({ email: v.email, views: v.views, lastViewed: formatTimeAgo(v.lastViewed), time: formatTime(v.totalTime), shares: v.shares.length }));

    const deviceCounts = { desktop: 0, mobile: 0, tablet: 0 };
    type DeviceType = 'desktop' | 'mobile' | 'tablet';
    oldViews.forEach((v: any) => {
      if (v.device && deviceCounts[v.device as DeviceType] !== undefined) deviceCounts[v.device as DeviceType]++;
    });
    ...
    const signingTimings = completedRecipients.filter(sr => sr.timeSpentSeconds != null).map(sr => sr.timeSpentSeconds);
    ...
    const startedSigning = signatureRequests.filter(sr => sr.status === 'completed' || sr.status === 'started').length;
    const signatureFriction = [
      { step: 'Document Opened', users: openedCount, avgTime: '0s', dropOff: 0 },
      {
        step: 'Scrolled to Signature', users: scrolledToSig,
        avgTime: formatTime(signatureRequests.filter(sr => sr.scrolledToSignatureAt && sr.firstViewedAt).reduce((sum: number, sr: any) => sum + Math.floor((new Date(sr.scrolledToSignatureAt).getTime() - new Date(sr.firstViewedAt).getTime()) / 1000), 0) / (signatureRequests.filter(sr => sr.scrolledToSignatureAt).length || 1)),
        dropOff: openedCount > 0 ? Math.round(((openedCount - scrolledToSig) / openedCount) * 100) : 0,
      },
      { step: 'Started Signing', users: startedSigning, avgTime: '0s', dropOff: scrolledToSig > 0 ? Math.round(((scrolledToSig - startedSigning) / scrolledToSig) * 100) : 0 },
      { step: 'Completed Signing', users: completedCount, avgTime: averageSigningTimeSeconds ? formatTime(averageSigningTimeSeconds) : '0s', dropOff: startedSigning > 0 ? Math.round(((startedSigning - completedCount) / startedSigning) * 100) : 0 },
    ];
    const eSignatureAnalytics = {
      totalRecipients, completedCount,
      ...
      recipientTimings: completedRecipients.filter(sr => sr.timeSpentSeconds != null).map(sr => ({
        recipient: sr.recipient?.name || sr.recipientEmail, email: sr.recipientEmail,
        timeSpent: sr.timeSpentSeconds, timeSpentFormatted: formatTime(sr.timeSpentSeconds), completedAt: sr.signedAt,
      })),
    ...
    const declineReasons = Array.from(declineReasonMap.entries())
      .map(([reason, count]) => ({ reason, count, percentage: totalDeclines > 0 ? Math.round((count / totalDeclines) * 100) : 0 }))
      .sort((a, b) => b.count - a.count);
    const declinePatterns = {
      avgTimeBeforeDecline: formatTime(declinedRequests.length > 0
        ? Math.round(declinedRequests.filter(sr => sr.declinedAt && sr.firstViewedAt).reduce((sum: number, sr: any) => sum + Math.floor((new Date(sr.declinedAt).getTime() - new Date(sr.firstViewedAt).getTime()) / 1000), 0) / (declinedRequests.filter(sr => sr.declinedAt && sr.firstViewedAt).length || 1))
        : 0),
      instantDeclines: declinedRequests.filter(sr => { if (!sr.declinedAt || !sr.firstViewedAt) return false; return (new Date(sr.declinedAt).getTime() - new Date(sr.firstViewedAt).getTime()) / 1000 < 30; }).length,
      deepReviewDeclines: declinedRequests.filter(sr => { if (!sr.declinedAt || !sr.firstViewedAt) return false; return (new Date(sr.declinedAt).getTime() - new Date(sr.firstViewedAt).getTime()) / 1000 > 300; }).length,
      recipients: declinedRequests.map(sr => ({
        email: sr.recipientEmail, name: sr.recipient?.name || sr.recipientEmail,
        reason: sr.declineReason || 'No reason given', declinedAt: sr.declinedAt,
        timeBeforeDecline: sr.declinedAt && sr.firstViewedAt ? formatTime(Math.floor((new Date(sr.declinedAt).getTime() - new Date(sr.firstViewedAt).getTime()) / 1000)) : 'N/A',
        pageDeclinedOn: sr.pageDeclinedOn || null,
      })),
    };

    const multipleViewers = recipientPageTracking.filter(r => r.pageData.reduce((sum, p) => sum + p.visits, 0) > 3).length;
    const longTimeViewers = recipientPageTracking.filter(r => r.totalTimeSeconds > 300).length;
    const returnVisitors = Array.from(viewerEmailMap.values()).filter(v => v.views > 1).length;
    const completedPageViewers = recipientPageTracking.filter(r => r.pageData.every(p => p.visited)).length;
    ...
    const highIntentCount = recipientPageTracking.filter(r => r.totalTimeSeconds > 300 && !r.bounced).length;
    const lowIntentCount = recipientPageTracking.filter(r => r.bounced || r.neverOpened).length;
    const intentData = {
      highIntent: highIntentCount,
      mediumIntent: Math.max(0, totalTrackedRecipients - highIntentCount - lowIntentCount),
      lowIntent: lowIntentCount,
      signals: intentSignals,
    };

    const reminderLogs = await db.collection('reminder_logs').find({ documentId: id }).toArray();
    const reminderGroups = new Map<number, { total: number; signed: number; times: number[] }>();
    for (const req of signatureRequests) {
      const remindersForReq = reminderLogs.filter(r => r.signatureRequestId?.toString() === req._id?.toString()).length;
      const bucket = Math.min(remindersForReq, 3);
      const existing = reminderGroups.get(bucket) || { total: 0, signed: 0, times: [] };
      existing.total++;
      if (req.status === 'completed') { existing.signed++; if (req.timeSpentSeconds) existing.times.push(req.timeSpentSeconds); }
      reminderGroups.set(bucket, existing);
    }
    const reminderEffectiveness = ['No Reminder', '1 Reminder', '2 Reminders', '3+ Reminders'].map((label, i) => {
      const group = reminderGroups.get(i) || { total: 0, signed: 0, times: [] };
      const avgTimeSeconds = group.times.length > 0 ? Math.round(group.times.reduce((a, b) => a + b, 0) / group.times.length) : 0;
      return { reminderType: label, signRate: group.total > 0 ? Math.round((group.signed / group.total) * 100) : 0, avgTime: avgTimeSeconds > 0 ? formatTime(avgTimeSeconds) : 'N/A', total: group.total, signed: group.signed };
    });

    let deadDealScore = 0;
    const deadDealSignals: { signal: string; weight: number; type: string }[] = [];
    if (declinedRequests.length > 0) { deadDealScore += 30; deadDealSignals.push({ signal: `Declined by ${declinedRequests.length} recipient(s)`, weight: 30, type: 'CRITICAL' }); }
    const lastActivity = tracking.lastViewed ? new Date(tracking.lastViewed) : oldViews[0]?.viewedAt ? new Date(oldViews[0].viewedAt) : null;
    if (lastActivity) {
      const daysSinceActivity = (Date.now() - lastActivity.getTime()) / (1000 * 60 * 60 * 24);
      if (daysSinceActivity > 14 && declinedRequests.length > 0) { deadDealScore += 25; deadDealSignals.push({ signal: `No activity for ${Math.floor(daysSinceActivity)} days after decline`, weight: 25, type: 'CRITICAL' }); }
      else if (daysSinceActivity > 30) { deadDealScore += 20; deadDealSignals.push({ signal: `No activity for ${Math.floor(daysSinceActivity)} days`, weight: 20, type: 'HIGH' }); }
    }
    if (bounceAnalytics.bounceRate > 60) { deadDealScore += 20; deadDealSignals.push({ signal: `High bounce rate: ${bounceAnalytics.bounceRate}%`, weight: 20, type: 'HIGH' }); }
    if (declinePatterns.deepReviewDeclines > 0) { deadDealScore += 15; deadDealSignals.push({ signal: `${declinePatterns.deepReviewDeclines} recipient(s) declined after 5+ min review`, weight: 15, type: 'HIGH' }); }
    const zeroEngagementPages = pageEngagement.filter(p => p.totalViews === 0 && p.page > 1).length;
    if (zeroEngagementPages > 0) { deadDealScore += 10; deadDealSignals.push({ signal: `${zeroEngagementPages} page(s) never viewed`, weight: 10, type: 'MEDIUM' }); }
    deadDealScore = Math.min(100, deadDealScore);
    const deadDealVerdict = deadDealScore >= 80 ? 'DEAD' : deadDealScore >= 60 ? 'AT_RISK' : deadDealScore >= 30 ? 'WATCH' : 'HEALTHY';
    const deadDeal = {
      score: deadDealScore, verdict: deadDealVerdict,
      confidence: Math.min(95, 50 + deadDealScore / 2),
      signals: deadDealSignals,
      recoveryProbability: Math.max(0, 100 - deadDealScore),
    ...
    const videoLogs = await db.collection('analytics_logs').find({
      documentId: id, action: { $in: ['video_watched', 'video_progress', 'video_replayed'] }
    }).toArray();
    const videoStatsMap: Record<string, any> = {};
    videoLogs.forEach((log: any) => {
      const key = `page-${log.pageNumber}`;
      if (!videoStatsMap[key]) videoStatsMap[key] = { page: log.pageNumber, totalWatches: 0, replays: 0, completions: [], viewers: new Set() };
      if (log.action === 'video_watched') videoStatsMap[key].totalWatches++;
      if (log.action === 'video_replayed') videoStatsMap[key].replays++;
      if (log.action === 'video_progress' && log.watchPercent) videoStatsMap[key].completions.push(log.watchPercent);
      if (log.email) videoStatsMap[key].viewers.add(log.email);
    });
    const videoStats = Object.values(videoStatsMap).map((v: any) => ({
      page: v.page, totalWatches: v.totalWatches, replays: v.replays, uniqueViewers: v.viewers.size,
      avgCompletion: v.completions.length > 0 ? Math.round(v.completions.reduce((a: number, b: number) => a + b, 0) / v.completions.length) : 0,
      replayRate: v.totalWatches > 0 ? Math.round((v.replays / v.totalWatches) * 100) : 0,
    })).sort((a: any, b: any) => a.page - b.page);

    const viewerVideoMap: Record<string, any> = {};
    videoLogs.forEach((log: any) => {
      const email = log.email;
      if (!email) return;
      if (!viewerVideoMap[email]) viewerVideoMap[email] = { email, pages: {} };
      const page = log.pageNumber;
      if (!viewerVideoMap[email].pages[page]) viewerVideoMap[email].pages[page] = { page, watchCount: 0, replays: 0, maxCompletion: 0, replayedAt: [], lastWatchedAt: null };
    ...
    const viewerVideoStats = Object.values(viewerVideoMap).map((viewer: any) => ({
      email: viewer.email,
      pages: Object.values(viewer.pages).map((p: any) => ({
        page: p.page, watchCount: p.watchCount, replays: p.replays, maxCompletion: p.maxCompletion,
        replayedAt: p.replayedAt, lastWatchedAt: p.lastWatchedAt,
        signal: p.replays >= 3 ? 'needs_clarification' : p.maxCompletion >= 75 ? 'understood' : p.watchCount > 0 ? 'partial' : 'not_watched',
      })).sort((a: any, b: any) => a.page - b.page),
      overallSignal: (() => {
        const pages = Object.values(viewer.pages) as any[];
    ...
    const reactionLogs = await db.collection('document_reactions').find({ documentId: id }).toArray();
    const clarityByPage: Record<number, { clear: number; confused: number }> = {};
    reactionLogs.filter((r: any) => r.type === 'page_clarity' || !r.type).forEach((r: any) => {
    ...
    const dealIntentResponses = reactionLogs.filter((r: any) => r.type === 'deal_intent').map((r: any) => ({
      email: r.email || null, response: r.reaction, sessionId: r.sessionId, createdAt: r.createdAt,
    }));
    const intentSummary: Record<string, number> = {};
    dealIntentResponses.forEach((r: any) => { intentSummary[r.response] = (intentSummary[r.response] || 0) + 1; });

    // ── NDA acceptances — advanced plan only ─────────────────────
    // Business plan gets full NDA certificate downloads.
    // Full plan sees acceptance records but no download button.
    const ndaAcceptances = analyticsLevel === 'advanced'
      ? await db.collection('nda_acceptances').find({ documentId: id }).sort({ timestamp: -1 }).toArray()
      : []

    return NextResponse.json({
      success: true,
      analyticsLevel,   // ← always tell the frontend what level this is
      analytics: {
        ...
        totalTimeAllSessions: formatTime(allSessions.reduce((sum: number, s: any) => sum + (s.duration || 0), 0)),
        completionRate, downloads, shares: totalShares,
        lastViewed: tracking.lastViewed || null,
        viewsByDate, pageEngagement, topViewers, recipientPageTracking,
