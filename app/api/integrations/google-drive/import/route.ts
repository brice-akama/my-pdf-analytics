...
export async function POST(request: NextRequest) {
  try {
    ...
    const importedContentType = fileResponse.headers.get('content-type')?.split(';')[0].trim() || '';

    if (!SUPPORTED_IMPORT_TYPES.has(importedContentType)) {
      console.log('❌ [GDRIVE IMPORT] Unsupported file type:', importedContentType);
      return NextResponse.json(
        {
   ...
   // ── Step 9: Convert to PDF if needed (matches upload/route.ts) ─────────
    // Determine fileType from validated content-type, then convert non-PDF
    // files before any PDF-specific processing runs.
    ...
    console.log("⚙️ [GDRIVE IMPORT] Converting + processing...");
