...
export async function GET(
  ...
  try {
    ...
    if (document.folder) {
      ...
      // ✅ NEW: Check if user can download (might be view-only)
      if (!folderAccess.permission.canDownload) {
        console.log('❌ Download not allowed for this user');
        return NextResponse.json({ 
          error: 'You have view-only access to this folder. Downloads are not allowed.' 
        }, { status: 403 });
      }

      console.log('✅ Folder access granted with download permission');
    }
