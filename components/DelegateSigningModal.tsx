...
interface DelegateSigningModalProps {
  ...
  onDelegated: () => void;
  renderAsDrawer?: boolean; // ⭐ NEW PROP
}
...
export const DelegateSigningModal: React.FC<DelegateSigningModalProps> = ({
  ...
  currentRecipient,
  onDelegated,
  renderAsDrawer = false, // ⭐ DEFAULT TO FALSE
}) => {
  ...
  // ⭐ THE FORM CONTENT (used by both drawer and modal)
  const formContent = (
  ...
  // ⭐ IF RENDERING AS DRAWER, RETURN JUST THE CONTENT
  if (renderAsDrawer) {
    return formContent;
  }

  // ⭐ OTHERWISE, RETURN THE FULL MODAL WRAPPER
  return (
