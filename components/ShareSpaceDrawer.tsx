...
// ─── Blocked feature shape returned by the backend ───────────────────────────
type BlockedFeature = {
  feature: string
  requiredPlan: string
  label: string
...
// ─── Helper: render the plan-limit toast with an upgrade CTA ─────────────────
//
// WHY A HELPER INSTEAD OF INLINE:
//   The same shape is needed for SHARE_LIMIT_REACHED and FEATURE_NOT_AVAILABLE.
//   Centralising it keeps every error visually consistent and easy to update.
//
// The toast uses sonner's JSX API so we can embed an "Upgrade" link button
// directly inside the notification rather than just plain text.
function showUpgradeToast(title: string, body: string) {
  toast.error(
    <div className="flex flex-col gap-1.5">
      <p className="font-semibold text-[13px] leading-snug">{title}</p>
      <p className="text-[12px] text-slate-300 leading-snug">{body}</p>
      <a
        href="/plan"
        className="mt-1 inline-flex items-center gap-1 text-[11px] font-semibold text-sky-400 hover:text-sky-300 transition-colors"
      >
        View upgrade options <ArrowUpRight className="h-3 w-3" />
      </a>
    </div>,
    {
      duration: 8000,
      // sonner's icon override — show a lock icon to reinforce "plan wall"
      icon: <Lock className="h-4 w-4 text-red-400 flex-shrink-0 mt-0.5" />,
    }
  )
}

// ─── Helper: parse the error response and fire the right toast ────────────────
//
// CALLED ONCE from handleGenerate's catch/else block.
// Handles every error code the backend can return:
//
//   SHARE_LIMIT_REACHED   → user hit their numeric share link cap
//   FEATURE_NOT_AVAILABLE → one or more Pro/Starter features were requested
//   anything else         → generic error toast (network, unknown server error)
//
// We also return a human-readable inline error string so the drawer can show
// it near the submit button — belt AND braces.
function handleApiError(data: any): string {
  // ── 1. Share link COUNT limit ──────────────────────────────────────────────
  if (data.code === "SHARE_LIMIT_REACHED") {
    const planLabel = PLAN_LABELS[data.currentPlan] ?? data.currentPlan ?? "current"
    const used  = data.used  ?? "?"
    const limit = data.limit ?? "?"

    showUpgradeToast(
      `Share link limit reached (${used}/${limit})`,
      `Your ${planLabel} plan allows ${limit} active share link${limit === 1 ? "" : "s"}. ` +
      `Upgrade to Starter or higher for unlimited share links.`
    )

    return `You've used all ${limit} share links on the ${planLabel} plan. Upgrade to create more.`
  }

  // ── 2. Feature flag gate — one or more Pro/Starter features blocked ────────
  if (data.code === "FEATURE_NOT_AVAILABLE" && Array.isArray(data.blockedFeatures)) {
    const blocked: BlockedFeature[] = data.blockedFeatures
    const planLabel = PLAN_LABELS[data.currentPlan] ?? data.currentPlan ?? "current"

    // Group blocked features by the minimum plan that unlocks them so the
    // toast reads "Watermarking, OTP (Pro)" not three separate lines.
    const byPlan: Record<string, string[]> = {}
    for (const f of blocked) {
      const p = PLAN_LABELS[f.requiredPlan] ?? f.requiredPlan
      if (!byPlan[p]) byPlan[p] = []
      byPlan[p].push(f.label)
    }

    const groupedLines = Object.entries(byPlan)
      .map(([plan, labels]) => `${labels.join(", ")} → requires ${plan}`)
      .join(" · ")

    showUpgradeToast(
      `Feature${blocked.length > 1 ? "s" : ""} not available on the ${planLabel} plan`,
      groupedLines
    )

    // Inline error — shorter version for inside the drawer
    const featureNames = blocked.map(f => f.label).join(", ")
    const minPlan = PLAN_LABELS[blocked[0].requiredPlan] ?? blocked[0].requiredPlan
    return `${featureNames} ${blocked.length > 1 ? "require" : "requires"} the ${minPlan} plan or higher. Turn ${blocked.length > 1 ? "them" : "it"} off or upgrade.`
  }

  // ── 3. Fallback — generic server / network error ───────────────────────────
  const msg = data.error || "Failed to create share link. Please try again."
  toast.error(msg)
  return msg
}
...
export function ShareSpaceDrawer({ open, onClose, spaceId, spaceName, onSuccess }: Props) {
  ...
  const [inlineError, setInlineError] = useState("") // ← shown near the submit button
  ...
  const set = useCallback(<K extends keyof ShareConfig>(k: K, v: ShareConfig[K]) => {
    ...
    // Clear the inline error whenever the user changes any field —
    // they may have just turned off the feature that was blocked.
    setInlineError("")
  }, [])
  ...
  async function handleGenerate() {
    ...
    setInlineError("")
    try {
      // ── Upload NDA if a new file was chosen ────────────────────────────────
      ...
      // ── Upload logo if branding is on ──────────────────────────────────────
      let logoUrl = null
      if (cfg.brandingEnabled && cfg.logoFile) {
        const lf = new FormData()
        lf.append("file", cfg.logoFile)
        lf.append("type", "logo")
        const lr = await fetch(`/api/spaces/${spaceId}/branding/logo`, {
          method: "POST", body: lf, credentials: "include",
        })
        const ld = await lr.json()
        if (ld.success) logoUrl = ld.url
      }

      // ── POST to public-access ──────────────────────────────────────────────
      ...
      // ── PLAN LIMIT / FEATURE GATE errors — structured response ────────────
      // handleApiError fires the correct toast (with upgrade CTA) AND returns
      // a short string for the inline error banner inside the drawer.
      if (res.status === 403 && (data.code === "SHARE_LIMIT_REACHED" || data.code === "FEATURE_NOT_AVAILABLE")) {
        const inline = handleApiError(data)
  ...
  function handleClose() {
    setStep(0); setSending(false); setShareUrl(""); setInlineError("")
    setEmailInput(""); setDomainInput("")
    setCfg({
      label: "", securityLevel: "password", password: "",
      allowedEmails: [], allowedDomains: [],
      expiresAt: "", viewLimit: "",
  ...
  return (
    <>
      ...
      >
          ...
          <AnimatePresence mode="wait">
            ...
            {step === 1 && (
              ...
              >
                ...
                <button
                  onClick={() => { setStep(0); setShareUrl(""); setInlineError("") }}
        ...
        {/* ── Footer ───────────────────────────────────────────────────────── */}
        <div className="border-t border-slate-100 px-7 py-5 bg-white">

          {/* ── INLINE ERROR BANNER ────────────────────────────────────────────
              Shown directly above the action buttons so the user sees it
              without scrolling. Only rendered when inlineError is non-empty.
              Includes an "Upgrade plan" link so they can act immediately.
          ── */}
          <AnimatePresence>
            {inlineError && step === 0 && (
              <motion.div
              ...
              >
                <div className="flex items-start gap-3 px-4 py-3 bg-red-50 border border-red-200 rounded-xl">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm text-red-800 font-medium leading-snug">{inlineError}</p>
                    <a
                      href="/plan"
                      className="mt-1 inline-flex items-center gap-1 text-xs font-semibold text-red-600 hover:text-red-700 underline underline-offset-2 transition-colors"
                    >
                      View upgrade options <ArrowUpRight className="h-3 w-3" />
                    </a>
                  </div>
