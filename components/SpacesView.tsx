...
import {
  Plus, Search, FolderOpen, Lock, Share2, MoreVertical,
  Briefcase, DollarSign, Target, Link as LinkIcon,
  Copy, Trash2, Archive, X, RotateCcw, Mail, Check,
} from "lucide-react"
      ...
      {open && (
        <>
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            onClick={() => onOpenChange(false)} className="fixed inset-0 bg-black/50 z-50" />
...
// ─── Confirm Modal ────────────────────────────────────────────────────────────
function ConfirmModal({ open, onOpenChange, title, description, confirmLabel = "Confirm",
  confirmClassName = "bg-gray-900 text-white hover:bg-gray-800", onConfirm }: {
  open: boolean; onOpenChange: (v: boolean) => void; title: string; description: string
  confirmLabel?: string; confirmClassName?: string; onConfirm: () => void
}) {
  return (
    <AnimatePresence>
      {open && (
        <>
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            className="fixed inset-0 bg-black/40 z-[60]" onClick={() => onOpenChange(false)} />
...
type SpaceType = {
  ...
  teamMembers: number; documentsCount: number; viewsCount: number
  lastActivity: string; createdAt: string; color: string; icon: string
...
// ─── SpacesView (renders inline — no page wrapper, no back button) ─────────────
export default function SpacesView() {
  ...
  const [confirmModal, setConfirmModal] = useState<{
    ...
    confirmLabel: string; confirmClassName: string; onConfirm: () => void
  }>({ open: false, title: '', description: '', confirmLabel: 'Confirm', confirmClassName: 'bg-gray-900 text-white hover:bg-gray-800', onConfirm: () => {} })
  ...
  const fetchSpaces = useCallback(async () => {
    try {
      ...
      if (data.success) {
        setAllSpaces(data.spaces.filter((s: any) => s.role === 'owner' || s.role === 'admin' || s.role === 'member' || s.isOwner))
        setAllMemberSpaces(data.spaces.filter((s: any) => s.role === 'viewer' || s.role === 'editor'))
      }
    } catch (err) {
  ...
  const filteredSpaces = allSpaces.filter(space => {
    ...
    return (!q || space.name.toLowerCase().includes(q) || (space.description || '').toLowerCase().includes(q))
      && (filterType === 'all' || space.status === filterType)
  })
  ...
  const openInviteDrawer = (space: SpaceType, e: React.MouseEvent) => {
    e.stopPropagation()
    setInviteTargetSpace(space); setInviteEmail(""); setInviteRole('viewer')
    setInviteMessage(""); setInviteSent(false); setShowInviteDrawer(true)
  }

  const handleSendInvite = async () => {
    if (!inviteEmail.trim() || !inviteTargetSpace) return
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(inviteEmail.trim())) { toast.error('Please enter a valid email'); return }
    setInviting(true)
    try {
      ...
      const data = await res.json()
      if (res.ok && data.success) { setInviteSent(true); toast.success(`Invite sent to ${inviteEmail.trim()}`) }
  ...
  const handleToggleSpace = async (spaceId: string, currentStatus: string, e: React.MouseEvent) => {
    ...
    try {
      ...
      if (res.ok && data.success) {
        setAllSpaces(prev => prev.map(s => s._id === spaceId ? { ...s, status: newStatus as SpaceType['status'] } : s))
  ...
  const handleDeleteSpace = (spaceId: string, spaceName: string, e: React.MouseEvent) => {
    e.stopPropagation()
    setConfirmModal({
      open: true, title: `Delete "${spaceName}"?`,
      description: 'This will permanently delete the space and all its documents. This cannot be undone.',
      confirmLabel: 'Delete Permanently', confirmClassName: 'bg-red-600 text-white hover:bg-red-700',
      onConfirm: async () => {
        try {
          const res = await fetch(`/api/spaces/${spaceId}`, { method: 'DELETE', credentials: 'include' })
          const data = await res.json()
          if (data.success) { setAllSpaces(prev => prev.filter(s => s._id !== spaceId)); toast.success('Space deleted') }
  ...
  const handleDuplicateSpace = async (space: SpaceType, e: React.MouseEvent) => {
    ...
    try {
      const res = await fetch('/api/spaces', {
        method: 'POST', credentials: 'include',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ name: `${space.name} (Copy)`, description: space.description, type: space.type, status: 'active', privacy: 'private', allowDownloads: true, notifyOnView: true, requireNDA: false, enableWatermark: false, autoExpiry: false }),
      })
      const data = await res.json()
      if (res.ok && data.success) {
        setAllSpaces(prev => [{ _id: data.space._id, name: data.space.name, description: data.space.description || '', type: data.space.type, status: 'active', teamMembers: 1, documentsCount: 0, viewsCount: 0, lastActivity: new Date().toISOString(), createdAt: new Date().toISOString(), color: space.color, icon: '', permissions: { canView: true, canEdit: true, canShare: true, canDownload: true } }, ...prev])
        toast.success('Space duplicated', { id: toastId })
      } else toast.error(data.error || 'Failed to duplicate', { id: toastId })
    } catch { toast.error('Failed to duplicate', { id: toastId }) }
  }
  ...
  const handleCreateSpace = async () => {
    ...
    try {
      ...
      } else toast.error(data.error || 'Failed to create space', { id: toastId })
  ...
  const iconBg: Record<string, string> = { deal: 'bg-blue-100 text-blue-600', fundraising: 'bg-green-100 text-green-600', client: 'bg-cyan-100 text-cyan-600', custom: 'bg-purple-100 text-purple-600' }
  const typeBadge: Record<string, string> = { deal: 'bg-blue-50 text-blue-700', fundraising: 'bg-green-50 text-green-700', client: 'bg-cyan-50 text-cyan-700', custom: 'bg-purple-50 text-purple-700' }
  const typeLabel: Record<string, string> = { deal: 'Deal', fundraising: 'Fundraising', client: 'Client', custom: 'Custom' }
  ...
  return (
    <div className="">
      {/* ── Inline Header (no back button — we're already in dashboard) ── */}
      <div className="flex items-center justify-between mb-6">
         
        <div className="flex items-center gap-2">
          <button onClick={() => setShowTemplatesDrawer(true)}
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 mb-5">
        ...
        <div className="flex items-center gap-1 p-1 bg-white border border-gray-200 rounded-lg">
          {([['all', `All (${allSpaces.length})`], ['active', `Active (${activeCount})`], ['archived', `Archived (${archivedCount})`]] as [FilterType, string][]).map(([f, label]) => (
            <button key={f} onClick={() => setFilterType(f)}
              className={`px-3 py-1 rounded-md text-xs font-medium transition-all ${filterType === f ? 'bg-gray-900 text-white shadow-sm' : 'text-gray-500 hover:text-gray-900'}`}>
              {label}
            </button>
          ))}
        </div>
      ...
      {filteredSpaces.length === 0 && filteredMemberSpaces.length === 0 && (
        <div className="bg-white border border-dashed border-gray-300 rounded-2xl p-16 text-center">
          ...
          <h3 className="text-base font-semibold text-gray-900 mb-1">
            {searchQuery ? 'No spaces match your search' : filterType === 'archived' ? 'No archived spaces' : filterType === 'active' ? 'No active spaces' : 'No data rooms yet'}
          </h3>
          <p className="text-sm text-gray-500 max-w-sm mx-auto mb-5">
            {searchQuery ? 'Try a different search term.' : filterType !== 'all' ? 'Try switching the filter above.' : 'Create a secure space to organize deals, fundraising, or client projects.'}
          </p>
          {!searchQuery && filterType === 'all' ? (
            <div className="flex gap-2 justify-center">
              <button onClick={() => setShowTemplatesDrawer(true)} className="px-4 py-2 text-sm border border-gray-200 text-gray-600 rounded-lg hover:border-gray-400 hover:text-gray-900 transition-all">Browse Templates</button>
              <button onClick={() => setShowCreateDrawer(true)} className="px-4 py-2 text-sm bg-gray-900 text-white font-medium rounded-lg hover:bg-gray-800 transition-colors">Create Space</button>
            </div>
          ) : (
            <button onClick={() => { setSearchQuery(''); setFilterType('all') }} className="text-sm text-gray-500 hover:text-gray-900 underline transition-colors">Clear filters</button>
          )}
        </div>
      )}
      ...
      {filteredSpaces.length > 0 && (
        <div className="mb-6">
              ...
              <thead>
                <tr className="border-b border-gray-100">
                  ...
                  {['Name', 'Type', 'Docs', 'Members', 'Last Activity', 'Status', ''].map((h, i) => (
                    <th key={i} className={`px-4 py-2.5 text-xs font-medium text-gray-400 uppercase tracking-wider ${i === 6 ? 'text-right' : 'text-left'}`}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-100">
                {filteredSpaces.map(space => (
                  <tr key={space._id} className={`hover:bg-gray-50 transition-colors cursor-pointer group ${space.status === 'archived' ? 'opacity-55' : ''}`}
                    ...
                    <td className="px-4 py-3.5">
                      <div className="flex items-center gap-3">
                        <div className={`h-8 w-8 rounded-lg flex items-center justify-center flex-shrink-0 ${iconBg[space.type] || iconBg.custom}`}><SpaceIcon type={space.type} /></div>
                        <div>
                    ...
                    <td className="px-4 py-3.5"><span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${typeBadge[space.type] || typeBadge.custom}`}>{typeLabel[space.type] || 'Custom'}</span></td>
                    ...
                    <td className="px-4 py-3.5" onClick={e => e.stopPropagation()}>
                      <div className="flex items-center gap-2">
                        <button onClick={e => handleToggleSpace(space._id, space.status, e)} disabled={togglingId === space._id}
                    ...
                    <td className="px-4 py-3.5 text-right" onClick={e => e.stopPropagation()}>
                      <div className="flex items-center justify-end gap-1">
                        {space.status === 'active' && (
                          <button onClick={e => { e.stopPropagation(); router.push(`/spaces/${space._id}`) }}
                        ...
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end" className="w-48 bg-white border-gray-200 shadow-lg">
                            {space.status === 'active' && (
                              <>
                                <DropdownMenuItem onClick={e => { e.stopPropagation(); router.push(`/spaces/${space._id}`) }} className="cursor-pointer text-sm"><FolderOpen className="mr-2 h-4 w-4" /> Open Space</DropdownMenuItem>
                                <DropdownMenuItem onClick={e => openInviteDrawer(space, e)} className="cursor-pointer text-sm"><Share2 className="mr-2 h-4 w-4" /> Share Access</DropdownMenuItem>
                                <DropdownMenuItem onClick={e => handleDuplicateSpace(space, e)} className="cursor-pointer text-sm"><Copy className="mr-2 h-4 w-4" /> Duplicate</DropdownMenuItem>
                                <DropdownMenuItem onClick={e => handleToggleSpace(space._id, space.status, e)} className="cursor-pointer text-sm"><Archive className="mr-2 h-4 w-4" /> Archive</DropdownMenuItem>
                              </>
                            )}
                            {space.status === 'archived' && (
                              <DropdownMenuItem onClick={e => handleToggleSpace(space._id, space.status, e)} className="cursor-pointer text-sm text-green-700 focus:text-green-700 focus:bg-green-50"><RotateCcw className="mr-2 h-4 w-4" /> Restore</DropdownMenuItem>
                            )}
                            ...
                            <DropdownMenuItem onClick={e => handleDeleteSpace(space._id, space.name, e)} className="text-red-600 focus:text-red-600 focus:bg-red-50 cursor-pointer text-sm">
                              <Trash2 className="mr-2 h-4 w-4" />{space.status === 'archived' ? 'Delete Permanently' : 'Delete'}
                            </DropdownMenuItem>
                          </DropdownMenuContent>
                        </DropdownMenu>
                      </div>
          ...
          <div className="lg:hidden divide-y divide-gray-100 border-t border-gray-100">
            {filteredSpaces.map(space => (
              <div key={space._id} className={`flex items-center gap-3 py-3.5 px-1 cursor-pointer hover:bg-gray-50 transition-colors ${space.status === 'archived' ? 'opacity-55' : ''}`}
                onClick={() => router.push(`/spaces/${space._id}`)}>
                <div className={`h-9 w-9 rounded-lg flex items-center justify-center flex-shrink-0 ${iconBg[space.type] || iconBg.custom}`}><SpaceIcon type={space.type} /></div>
                ...
                <div className="flex items-center gap-2 flex-shrink-0" onClick={e => e.stopPropagation()}>
                  <span className={`text-xs font-medium ${space.status === 'active' ? 'text-green-600' : 'text-gray-400'}`}>{space.status === 'active' ? 'Active' : 'Archived'}</span>
                  <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                      <button className="p-1.5 text-gray-400 hover:text-gray-700 rounded-md hover:bg-gray-100 transition-all"><MoreVertical className="h-4 w-4" /></button>
                    </DropdownMenuTrigger>
                    <DropdownMenuContent align="end" className="w-48 bg-white border-gray-200 shadow-lg">
                      {space.status === 'active' && (
                        <>
                          <DropdownMenuItem onClick={() => router.push(`/spaces/${space._id}`)} className="cursor-pointer text-sm"><FolderOpen className="mr-2 h-4 w-4" /> Open Space</DropdownMenuItem>
                          <DropdownMenuItem onClick={e => openInviteDrawer(space, e)} className="cursor-pointer text-sm"><Share2 className="mr-2 h-4 w-4" /> Share Access</DropdownMenuItem>
                          <DropdownMenuItem onClick={e => handleDuplicateSpace(space, e)} className="cursor-pointer text-sm"><Copy className="mr-2 h-4 w-4" /> Duplicate</DropdownMenuItem>
                          <DropdownMenuItem onClick={e => handleToggleSpace(space._id, space.status, e)} className="cursor-pointer text-sm"><Archive className="mr-2 h-4 w-4" /> Archive</DropdownMenuItem>
                        </>
          ...
          <div className="border-t border-gray-100 px-1 pt-2.5 flex items-center justify-between">
            ...
            <p className="text-xs text-gray-400">{filteredSpaces.reduce((n, s) => n + (s.documentsCount || 0), 0)} docs · {filteredSpaces.reduce((n, s) => n + (s.viewsCount || 0), 0)} views</p>
          </div>
        </div>
      )}
      ...
      {filteredMemberSpaces.length > 0 && (
        <div>
              ...
              <thead>
                <tr className="border-b border-gray-100">
                  ...
                  {['Name', 'Your Role', 'Docs', 'Members', ''].map((h, i) => (
                    <th key={h} className={`px-4 py-2.5 text-xs font-medium text-gray-400 uppercase tracking-wider ${i === 4 ? 'text-right' : 'text-left'}`}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-100">
                {filteredMemberSpaces.map(space => (
                  <tr key={space._id} className="hover:bg-gray-50 transition-colors cursor-pointer group" onClick={() => router.push(`/spaces/${space._id}`)}>
                    <td className="px-4 py-3.5" onClick={e => e.stopPropagation()}><input type="checkbox" className="rounded border-gray-300" /></td>
                    <td className="px-4 py-3.5">
                      <div className="flex items-center gap-3">
                    ...
                    <td className="px-4 py-3.5"><span className="text-xs font-medium text-slate-600 capitalize">{space.role}</span></td>
                    <td className="px-4 py-3.5 text-sm text-gray-700">{space.documentsCount || 0}</td>
                    <td className="px-4 py-3.5 text-sm text-gray-700">{space.teamMembers || 0}</td>
                    <td className="px-4 py-3.5 text-right" onClick={e => e.stopPropagation()}>
                      <button onClick={e => { e.stopPropagation(); router.push(`/spaces/${space._id}`) }}
          ...
          <div className="lg:hidden divide-y divide-gray-100 border-t border-gray-100">
            {filteredMemberSpaces.map(space => (
              <div key={space._id} className="flex items-center gap-3 py-3.5 px-1 cursor-pointer hover:bg-gray-50 transition-colors" onClick={() => router.push(`/spaces/${space._id}`)}>
      ...
      {/* ── Quick Invite Drawer ── */}
      <Drawer open={showInviteDrawer} onOpenChange={open => { setShowInviteDrawer(open); if (!open) { setInviteTargetSpace(null); setInviteSent(false); setInviteEmail("") } }}>
        <div className="flex items-start justify-between mb-6">
          <div>
            <h2 className="text-lg font-semibold text-gray-900">Share Access</h2>
            {inviteTargetSpace && (
              <div className="flex items-center gap-2 mt-1.5">
                <div className={`h-5 w-5 rounded flex items-center justify-center ${iconBg[inviteTargetSpace.type] || iconBg.custom}`}><SpaceIcon type={inviteTargetSpace.type} /></div>
                <p className="text-sm text-gray-500 truncate max-w-[260px]">{inviteTargetSpace.name}</p>
              </div>
            )}
          </div>
          <button onClick={() => setShowInviteDrawer(false)} className="p-1.5 text-gray-400 hover:text-gray-700 rounded-lg hover:bg-gray-100 transition-all"><X className="h-5 w-5" /></button>
        </div>
        <AnimatePresence mode="wait">
          {!inviteSent ? (
            <motion.div key="form" initial={{ opacity: 0, x: 12 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -12 }} transition={{ duration: 0.15 }} className="space-y-5">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1.5">Email address *</label>
                <div className="relative">
                  <Mail className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-gray-400" />
                  <input type="email" placeholder="colleague@company.com" value={inviteEmail} onChange={e => setInviteEmail(e.target.value)} onKeyDown={e => { if (e.key === 'Enter' && inviteEmail.trim()) handleSendInvite() }} autoFocus
                ...
                <div className="space-y-2">
                  {ROLES.map(role => (
                    <button key={role.value} onClick={() => setInviteRole(role.value)}
                      className={`w-full flex items-center gap-3 px-3.5 py-3 rounded-xl border transition-all text-left ${inviteRole === role.value ? 'border-gray-900 bg-gray-50 ring-1 ring-gray-900' : 'border-gray-200 hover:border-gray-300'}`}>
                      <span className="text-base">{role.icon}</span>
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-medium text-gray-900">{role.label}</p>
                        <p className="text-xs text-gray-500">{role.desc}</p>
                      </div>
                      {inviteRole === role.value && <div className="h-4 w-4 rounded-full bg-gray-900 flex items-center justify-center flex-shrink-0"><Check className="h-2.5 w-2.5 text-white" /></div>}
                    </button>
                  ))}
                </div>
              </div>
              <div>
                ...
                <textarea placeholder="Add a personal note…" rows={3} value={inviteMessage} onChange={e => setInviteMessage(e.target.value)}
              ...
              <div className="flex gap-2 pt-1">
                <button onClick={() => setShowInviteDrawer(false)} className="flex-1 px-4 py-2.5 text-sm text-gray-600 border border-gray-200 hover:border-gray-300 rounded-xl transition-all">Cancel</button>
                <button onClick={handleSendInvite} disabled={inviting || !inviteEmail.trim()}
                  ...
                  {inviting ? <><div className="h-3.5 w-3.5 border-2 border-white/30 border-t-white rounded-full animate-spin" />Sending…</> : <><Mail className="h-3.5 w-3.5" />Send Invite</>}
                </button>
              </div>
            </motion.div>
          ) : (
            <motion.div key="success" initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.2 }} className="text-center py-6">
              <div className="h-14 w-14 rounded-full bg-green-100 flex items-center justify-center mx-auto mb-4"><Check className="h-7 w-7 text-green-600" /></div>
              <h3 className="text-base font-semibold text-gray-900 mb-1">Invite sent!</h3>
              <p className="text-sm text-gray-500 mb-6"><span className="font-medium text-gray-700">{inviteEmail}</span> will receive access to <span className="font-medium text-gray-700">{inviteTargetSpace?.name}</span>.</p>
              <div className="flex gap-2 justify-center">
                <button onClick={() => { setInviteEmail(""); setInviteMessage(""); setInviteSent(false) }} className="px-4 py-2 text-sm border border-gray-200 text-gray-600 hover:border-gray-400 rounded-xl transition-all">Invite Another</button>
                <button onClick={() => setShowInviteDrawer(false)} className="px-4 py-2 text-sm bg-gray-900 text-white font-medium rounded-xl hover:bg-gray-800 transition-colors">Done</button>
              </div>
            </motion.div>
          )}
        </AnimatePresence>
      ...
      <Drawer open={showTemplatesDrawer} onOpenChange={setShowTemplatesDrawer}>
        <div className="flex items-start justify-between mb-6">
          ...
          <button onClick={() => setShowTemplatesDrawer(false)} className="p-1.5 text-gray-400 hover:text-gray-700 rounded-lg hover:bg-gray-100 transition-all"><X className="h-5 w-5" /></button>
        </div>
        <div className="grid grid-cols-2 gap-3">
          {TEMPLATES.map(t => (
            <div key={t.id} className="border border-gray-200 rounded-xl p-4 hover:border-gray-400 hover:shadow-sm transition-all cursor-pointer group"
              onClick={() => { setNewSpace({ ...newSpace, name: t.name, type: t.type as SpaceType['type'], template: t.id }); setShowTemplatesDrawer(false); setShowCreateDrawer(true) }}>
              <div className={`h-10 w-10 rounded-xl bg-gradient-to-br ${t.color} flex items-center justify-center text-xl mb-3`}>{t.icon}</div>
              <h3 className="font-semibold text-gray-900 mb-1 group-hover:text-blue-600 transition-colors text-sm">{t.name}</h3>
              <p className="text-xs text-gray-500 mb-2">{t.description}</p>
              <p className="text-xs text-gray-400">Includes: {t.folders.slice(0, 2).join(', ')}{t.folders.length > 2 && ` +${t.folders.length - 2} more`}</p>
            </div>
          ))}
        </div>
      </Drawer>
      ...
      {/* ── Create Space Drawer ── */}
      <Drawer open={showCreateDrawer} onOpenChange={open => { setShowCreateDrawer(open); if (!open) setNewSpace(EMPTY_FORM) }}>
        <div className="flex items-start justify-between mb-6">
          <div>
          ...
          <button onClick={() => { setShowCreateDrawer(false); setNewSpace(EMPTY_FORM) }} className="p-1.5 text-gray-400 hover:text-gray-700 rounded-lg hover:bg-gray-100 transition-all"><X className="h-5 w-5" /></button>
        </div>
        <Tabs defaultValue="basic" className="w-full">
          ...
          <TabsContent value="basic" className="space-y-4">
            <div>
              ...
              <input placeholder="e.g., Series A Fundraising Room" value={newSpace.name} onChange={e => setNewSpace({ ...newSpace, name: e.target.value })}
            ...
            <div>
              ...
              <textarea placeholder="What is this space for?" rows={3} value={newSpace.description} onChange={e => setNewSpace({ ...newSpace, description: e.target.value })}
          ...
          <TabsContent value="security" className="space-y-0">
            {[
              ...
              <div key={key} className="flex items-center justify-between py-3.5 border-b border-gray-100 last:border-0">
                ...
                <Switch checked={newSpace[key as keyof typeof newSpace] as boolean} onCheckedChange={v => setNewSpace({ ...newSpace, [key]: v })} />
              </div>
            ))}
            {newSpace.autoExpiry && (
              <div className="pt-3">
                <Label className="text-sm font-medium text-gray-700 mb-1.5 block">Expiry Date</Label>
                <input type="date" value={newSpace.expiryDate} onChange={e => setNewSpace({ ...newSpace, expiryDate: e.target.value })}
          ...
          <TabsContent value="permissions" className="space-y-4">
            <div className="flex items-center justify-between py-3.5 border-b border-gray-100">
              <div><p className="text-sm font-medium text-gray-900">Allow Downloads</p><p className="text-xs text-gray-500 mt-0.5">Users can download documents</p></div>
              <Switch checked={newSpace.allowDownloads} onCheckedChange={v => setNewSpace({ ...newSpace, allowDownloads: v })} />
            </div>
            <div>
              ...
              <div className="space-y-2">
                {[
                  ...
                  <button key={value} onClick={() => setNewSpace({ ...newSpace, privacy: value })}
                    className={`w-full flex items-center gap-3 p-3.5 border rounded-xl transition-all text-left ${newSpace.privacy === value ? 'border-gray-900 bg-gray-50 ring-1 ring-gray-900' : 'border-gray-200 hover:border-gray-300'}`}>
                    <Icon className="h-4 w-4 text-gray-500 flex-shrink-0" />
                    <div><div className="text-sm font-medium text-gray-900">{title}</div><div className="text-xs text-gray-500">{desc}</div></div>
                  </button>
                ))}
              </div>
            </div>
        ...
        <div className="flex justify-end gap-2 mt-6 pt-4 border-t border-gray-100">
          <button onClick={() => { setShowCreateDrawer(false); setNewSpace(EMPTY_FORM) }} className="px-4 py-2 text-sm text-gray-600 hover:text-gray-900 border border-gray-200 hover:border-gray-300 rounded-lg transition-all">Cancel</button>
          <button onClick={handleCreateSpace} disabled={creating || !newSpace.name.trim()}
      ...
      {/* ── Confirm Modal ── */}
      <ConfirmModal open={confirmModal.open} onOpenChange={open => setConfirmModal(prev => ({ ...prev, open }))}
        title={confirmModal.title} description={confirmModal.description}
        confirmLabel={confirmModal.confirmLabel} confirmClassName={confirmModal.confirmClassName}
