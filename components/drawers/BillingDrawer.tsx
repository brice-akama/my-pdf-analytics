...
//
// WHAT THIS FILE DOES:
//   The billing drawer accessible from the dashboard top nav.
//   Shows current plan, subscription status, storage usage, and lets
//   the user cancel their subscription without leaving DocMetrics.
//
// PHASE 5 CHANGES:
//   - Removed the broken customer.paddle.com external link
//     (it only works in Paddle production, not sandbox, and sends users off-platform)
//   - Added in-app "Cancel plan" button that calls POST /api/paddle/cancel
//   - Cancel flow has a confirmation step showing exactly what happens
//   - After cancellation, shows a success state with the access end date
//   - All billing data comes from getUserBilling() with cache: 'no-store'
//     so it always reflects the most recent webhook update
//
// THREE VIEWS:
//   "main"           — default view, shows plan/storage/usage
//   "cancel_confirm" — confirmation step before calling the cancel API
//   "cancel_success" — shown after successful cancellation
    ...
    <>
      ...
      <div className="fixed right-0 top-0 bottom-0 w-full sm:w-[480px] bg-white shadow-2xl z-50 flex flex-col">
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b">
          ...
          <button onClick={onClose} className="h-8 w-8 rounded-full hover:bg-slate-100 flex items-center justify-center transition-colors">
          ...
          {!loading && view === "cancel_success" && (
            <div className="space-y-6">
              <div className="flex flex-col items-center text-center py-8">
                ...
                <h3 className="text-xl font-semibold text-slate-900 mb-2">Done — subscription canceled</h3>
              ...
              <div className="bg-amber-50 border border-amber-200 rounded-xl p-4">
                <p className="text-sm text-amber-800">
                  Your documents and settings are kept permanently. You can resubscribe anytime
          ...
          {!loading && view === "cancel_confirm" && (
            <div className="space-y-6">
              <div className="bg-slate-50 border border-slate-200 rounded-xl p-5">
                <h3 className="text-sm font-semibold text-slate-900 mb-3">What happens when you cancel</h3>
            ...
            <div className="space-y-5">
              ...
              {status === "canceled" && billing.currentPeriodEnd && (
                <div className="flex items-start gap-3 px-4 py-3 bg-amber-50 border border-amber-200 rounded-xl">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-amber-600 mt-0.5">Access until {formatDate(billing.currentPeriodEnd)}</p>
                  </div>
                </div>
              )}
              {status === "past_due" && (
                <div className="flex items-start gap-3 px-4 py-3 bg-red-50 border border-red-200 rounded-xl">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-red-600 mt-0.5">Paddle is retrying automatically.</p>
                  </div>
                </div>
              )}
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <div className="flex items-center gap-2">
                        <h3 className={`text-xl font-bold ${planInfo.color}`}>{planInfo.label}</h3>
                        <span className={`px-2 py-0.5 rounded-full text-xs font-semibold ${
                          status === "active" && !billing.isTrialActive ? "bg-emerald-100 text-emerald-700" :
                          billing.isTrialActive ? "bg-blue-100 text-blue-700" :
                          status === "canceled" ? "bg-amber-100 text-amber-700" :
                ...
                <div className="px-5 py-3 space-y-2">
                  {billing.billingCycle && plan !== "free" && (
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="font-medium text-slate-900 capitalize">{billing.billingCycle}</span>
                    </div>
                  )}
                  {billing.currentPeriodEnd && status === "active" && !billing.isTrialActive && (
                    <div className="flex justify-between text-sm">
                      <span className="text-slate-500">Next payment</span>
                      <span className="font-medium text-slate-900">{formatDate(billing.currentPeriodEnd)}</span>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="border border-slate-200 rounded-xl p-5">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <p className="text-xs text-slate-500">{formatBytes(storageUsed)} of {formatBytes(storageLimit)}</p>
                </div>
                <div className="h-1.5 bg-slate-100 rounded-full overflow-hidden">
                  <div
                    className={`h-full rounded-full transition-all ${storagePercent >= 95 ? "bg-red-500" : storagePercent >= 80 ? "bg-amber-500" : "bg-indigo-500"}`}
                    style={{ width: `${storagePercent}%` }}
                  />
                </div>
                {storagePercent >= 80 && (
                  <p className={`text-xs mt-1.5 ${storagePercent >= 95 ? "text-red-600" : "text-amber-600"}`}>
                    {storagePercent >= 95 ? "Storage almost full — upgrade or delete files" : "Storage running low"}
                  </p>
                )}
              </div>
              ...
              <div className="border border-slate-200 rounded-xl p-5">
                ...
                <div className="space-y-3">
                  {[
                    { label: "Documents", value: limits.maxDocuments === -1 ? `${documents?.length ?? 0} (unlimited)` : `${documents?.length ?? 0} / ${limits.maxDocuments}` },
                    { label: "Team seats", value: limits.maxTeamMembers === 1 ? "Solo" : `Up to ${limits.maxTeamMembers}` },
                    { label: "eSignatures / month", value: limits.maxESignaturesPerMonth === -1 ? "Unlimited" : String(limits.maxESignaturesPerMonth) },
                    { label: "Spaces", value: limits.maxSpaces === -1 ? "Unlimited" : String(limits.maxSpaces) },
                  ].map(({ label, value }) => (
          ...
          <div className="px-6 py-4 border-t bg-white">
            {plan === "free" || billing?.isTrialActive || status === "canceled" ? (
              ...
              >
                ...
                {status === "canceled" ? "Resubscribe" : billing?.isTrialActive ? "Choose a plan" : "Upgrade plan"}
              </button>
            ) : (
              <button onClick={onClose} className="w-full py-3 px-4 bg-slate-100 hover:bg-slate-200 text-slate-700 font-semibold rounded-xl transition-colors">
                Done
              </button>
            )}
          </div>
